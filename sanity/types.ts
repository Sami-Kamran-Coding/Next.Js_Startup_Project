...
// Query: *[    _type == "startup" &&    defined(slug.current) &&    (!defined($search) || title match $search || category match $search)  ] | order(_createdAt desc) {    _id,    _type,    _rev,    _createdAt,    _updatedAt,    title,    slug,    author, // keep as reference    views,    description,    category,    image,    pitch  }
...
// Query: *[_type == "playlist" && slug.current == $slug][0]{    _id,    title,    slug,    select[]->{      _id,      _type,      _rev,      _updatedAt,      _createdAt,      title,      slug,      author->{        _id,        name,        slug,        image,        bio      },      views,      description,      category,      image,      pitch    }  }
